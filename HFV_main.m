...
ctrler_type = 4; %%控制器类型选择标志位，1为校正网络,2为普通单环滑模控制,3为普通双环滑模控制,4为预设性能滑模控制
if sim_case == 2 
    h_step = 0.1; %%单位 s
else 
...
if sim_case == 1  
 %%%%%%%% 进行六自由度仿真，设计控制器，进行姿态控制 %%%%       
    ...
    Buf4 = zeros(1,34); %用于记录仿真数据的大数组
    SMCDataRec1 = zeros((T_term + h_step)/h_step,34);%用于记录仿真数据的多维大数组
    Buf3 = zeros(1,25); %用于记录仿真数据的大数组
    SMCDataRec2 = zeros((T_term + h_step)/h_step,25);%用于记录仿真数据的多维大数组
    Buf5 = zeros(1,31); %用于记录仿真数据的大数组
    SMCDataRec3 = zeros((T_term + h_step)/h_step,31);%用于记录仿真数据的多维大数组    
    Buf6 = zeros(1,13); %用于记录仿真数据的大数组
    PPFDataRec = zeros((T_term + h_step)/h_step,13);%用于记录仿真数据的多维大数组
...
%             count = count+1;
%             disp(Time);
%             LnrParaBufb = cal_LinearPara(DeltaA, Env_para, FltSdt_S, FltSdt_F);  
...
%         EulerAng_1 = RK_EulrAng(EulerAng, Omega, h_step); %数值积分求解下一时刻的欧拉角      
        ...
        elseif ctrler_type==3
            AfnPara = AfnNonlnrPara(DeltaA, Env_para, FltSdt_S, FltSdt_F); 
           [Eror_ang_1, Eror_omg_1, DeltaA_1,SMCData1] = Ctrller_SMC_Db(AfnPara, CmdAng_1, AeroAng_1, Omega_1); %%使用sliding mode 滑模控制器 
            Buf4 = [(Time + h_step),SMCData1]; SMCDataRec1(step,:)= Buf4;%用于记录仿真数据的多维大数组     
        elseif ctrler_type==4
             AfnPara = AfnNonlnrPara(DeltaA, Env_para, FltSdt_S, FltSdt_F); 
             [Eror_ang_1, Eror_omg_1, DeltaA_1,SMCData3,PPFData] = Ctrller_PPFSMC(AfnPara, CmdAng_1, AeroAng_1, Omega_1); %%使用具有预设性能的滑模控制
             Buf5 = [(Time + h_step),SMCData3];         SMCDataRec3(step,:)= Buf5;%用于记录仿真数据的多维大数组           
             Buf6 = [(Time + h_step), Eror_ang_1(1),PPFData,Eror_omg_1(1)];    PPFDataRec(step,:)= Buf6;%用于记录仿真数据的多维大数组
        else
...
%         end
    ...
    save('FltData2_SMC.mat','FltData2');
    %  save('SMCData1.mat','SMCDataRec1');
      save('SMCData2.mat','SMCDataRec2');
    save('SMCData3.mat','SMCDataRec3');
    save('PPFData.mat','PPFDataRec');
